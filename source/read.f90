...
!********************************************************************************************************
     subroutine read_parameters  ( geo, advection, dispersion, mass_trans, reaction, plume, well, plane,  &
	                               histo, source, recirculation, StressTimes )
	 ...
	 integer                           :: i,ivar,iwell,nwell,iplane,nplane,flag,ninj,kinj,iunit
	 integer                           :: iunitVEL
...
!     Read time parameters
      ...
      read(10,*) trans_flag, read_time_file_flag     !trans_flag: T if any parameter other than MF2K fluxes is transient
      if (trans_flag) then
          if (read_time_file_flag==1) then 
              read(10,*) file
              call get_trans_part_times_from_file (file)
          elseif (.not.read_time_file_flag==0) then
              read(10,*) num_stp
              allocate (transpar_times(num_stp))
              do i=1,num_stp; read(10,*) transpar_times(i); end do
          end if
      end if 
...
!     Read Advection Parameters:
      ...
      read(10,*) file,const,ivar,flag,trans
           dataqx%file  = file
           dataqx%const = const
           dataqx%ivar  = ivar
           dataqx%flag  = flag
     ...
     !...Initialize velocity field
      ...
      if (dataqx%flag==2 .OR. dataqy%flag==2 .OR. dataqz%flag==2) then !work with Modflow binary cell-by-cell flux file
          !Initialize modflow variables
          if (advection%action .and. flag == 2) then     
             call allocate_mf2k_ (mf2k)
             call check_budget_file_ (file,mf2k)
          end if    
	      read(10,*) nper, velotime%loop_per 
          ...
          !Default values
          allocate(velotime%period(1))
          velotime%period(1)%PERLEN = 1
	      velotime%period(1)%NSTP   = 1
	      velotime%period(1)%MULT   = 1
      end if
      
      call calculate_advection_timeshots_ (advection)
      
!----------------------
!     Read porosity / water content
      
      read(10,*) file,const,ivar,flag,trans
          advection%poro_info%file  = file
          advection%poro_info%const = const
          advection%poro_info%ivar  = ivar
          advection%poro_info%flag  = flag
          advection%poro_info%trans = trans
          
          if (.not.advection%poro_info%trans) advection%poro = read_array_ (file,ivar,const,flag,nx,ny,nz)
...
!     Read Dispersion Parameters:
	  ...
	  read(10,*) file,const,ivar,flag,trans
          dispersion%dm_info%file  = file
          dispersion%dm_info%const = const
          dispersion%dm_info%ivar  = ivar
          dispersion%dm_info%flag  = flag
          dispersion%dm_info%trans = trans
          if (dispersion%action .and. .not.dispersion%dm_info%trans) dispersion%dm = read_array_ (file,ivar,const,flag,nx,ny,nz)
...
!************************************************************************************************************************************************************************************
!                 READ TRANSIENT PARAMETERS TEMP. DISCRETIZATION FROM FILE
!************************************************************************************************************************************************************************************ 
  subroutine get_trans_part_times_from_file (filename)
  use trans_par_times
  use gslib, only: generate_unit
  implicit none
