...
module timefunction_class
	...
	public ::                                             & ! methods
              ...
              evaluate_timefunction_upwind_cell_          , &
              print_time_function_                 
	
    type val_cell_cl
         integer*8,  pointer  :: ncell    => null()  !number of cell 
         integer*8,  pointer  :: ix(:)    => null()  !cell index in x
         integer*8,  pointer  :: iy(:)    => null()  !cell index in y
         integer*8,  pointer  :: iz(:)    => null()  !cell index in z
         real*8,     pointer  :: val_c(:) => null()  !mass flux values (for each cell)
    end type
    ...
    type timefunction_cl    
         ...
         real*8,  pointer  :: val(:)   => null()  !mass flux values (for entire source)
         type(val_cell_cl), pointer, dimension(:)  :: val_cell   => null()
    end type      
   ...
   function evaluate_timefunction_upwind_cell_ (func,it) result(val)
       implicit none
       type(timefunction_cl), intent(in) :: func
       integer,               intent(in) :: it
       ...
       allocate(val(func%val_cell(it)%ncell))
       ...
       if (it<=nt) then 
           val = func%val_cell(it)%val_c
           val = val * func%scale
       else if(it>nt) then
           val = func%val_cell(it)%val_c ! continue by projection
           val = val * func%scale
       end if 
