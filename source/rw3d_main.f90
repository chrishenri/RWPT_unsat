...
!    _____________________________________________________________________________________________________________________
	 ...
	 logical                    :: remove,update_transpar
!    _____________________________________________________________________________________________________________________

!....read parameters and initialize:

	 call print_version   ( version, date )
     call read_parameters ( geo, advection, dispersion, mass_trans, reaction, plume, well, plane, histo, source, recirculation, StressTimes )
...
!.....Start loops:
     ...
     moves: do nmove=1,nmaxmove     
        ...
        !...update (if needed) all transient parameters vectors

        if (nmove>1) call check_update_transpar ( plume%time )
        if (transpar_switch) call update_transpar_ ( geo, advection, dispersion )

        !...read flow and inject particles:
        call read_fluxes_  ( advection, geo )                                                                               !read fluxes if flow changes 

        !call read_water_content_  ( advection, geo )
...
!****************************************************************************************************
     subroutine check_update_transpar(time)
        use trans_par_times
        implicit none
        real*8,  intent(in)  :: time
        
        if (trans_flag) then
            if (time >= transpar_times(transpar_it) .and. time < transpar_times(transpar_it+1)) then
                transpar_switch = .FALSE.
            else
                transpar_switch = .TRUE.
                transpar_it = transpar_it+1
            end if
        end if
...
!****************************************************************************************************
    ...
    subroutine update_transpar_ (geo,advection,dispersion)
        use advection_class
        use dispersion_class
        use geometry_class
        use array_class
        ...
        type(dispersion_cl), intent(inout) :: dispersion
        type(geometry_cl),  intent(in)     :: geo

        !update porostity / water content
        if (trans_flag) then
            if (advection%poro_info%trans) then
                if (associated(advection%poro%values)) call delete_array_ (advection%poro)
                call read_array_2_ (advection%poro_info%file,advection%poro_info%ivar,advection%poro_info%const,advection%poro_info%flag,geo%nx,geo%ny,geo%nz,advection%poro,advection%poro_info%iunit)
                    write(*,*) 'Update poro at t=',transpar_times(transpar_it)
            end if
            
            if (dispersion%dm_info%trans) then
                if (associated(dispersion%dm%values)) call delete_array_ (dispersion%dm)
                call read_array_2_ (dispersion%dm_info%file,dispersion%dm_info%ivar,dispersion%dm_info%const,dispersion%dm_info%flag,geo%nx,geo%ny,geo%nz,dispersion%dm,dispersion%dm_info%iunit)
                    write(*,*) 'Update Dm at t=',transpar_times(transpar_it)
            end if
               
            if (dispersion%dmTH_info%trans) then
                if (associated(dispersion%dmTH%values)) call delete_array_ (dispersion%dmTH)
                call read_array_2_ (dispersion%dmTH_info%file,dispersion%dmTH_info%ivar,dispersion%dmTH_info%const,dispersion%dmTH_info%flag,geo%nx,geo%ny,geo%nz,dispersion%dmTH,dispersion%dmTH_info%iunit)
                    write(*,*) 'Update DmTH at t=',transpar_times(transpar_it)
            end if

            if (dispersion%dmTV_info%trans) then
                if (associated(dispersion%dmTV%values)) call delete_array_ (dispersion%dmTV)
                call read_array_2_ (dispersion%dmTV_info%file,dispersion%dmTV_info%ivar,dispersion%dmTV_info%const,dispersion%dmTV_info%flag,geo%nx,geo%ny,geo%nz,dispersion%dmTV,dispersion%dmTV_info%iunit)
                    write(*,*) 'Update DmTV at t=',transpar_times(transpar_it)
            end if
