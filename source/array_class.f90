...
!*************************************************************************************************
    ...
    !***************************************************************************
    subroutine read_array_2_ (file_name,ivar,const,flag,nx,ny,nz,name,iunit)
    use gslib, only: generate_unit
    use trans_par_times
	implicit none
      integer,          intent(in)      :: nx,ny,nz,ivar,flag
	  real*8,           intent(in)      :: const
      character(len=*), intent(in)      :: file_name
      type(array_cl)  , intent(out)     :: name
      integer         , intent(inout)   :: iunit
      integer                           :: i,j,k,jcol !nvar
      real*8, allocatable               :: aline(:)
      logical                           :: exists
	  integer                           :: nline,iline
	  integer, save                     :: npriorline, nvar
	  real*8                            :: val
      character(len=100)                :: temp
          ...
          if (flag == 0 ) then              ! parameter is constant and is not necessary 
	         ...
	         if (transpar_it==1) then
                ...
                iunit = generate_unit(100)
                open(iunit,file=file_name)
                read(iunit,'(a30)',err=10,end=11) temp
                read(iunit,*,err=10,end=11) nvar
             end if
             !read(iunit,*,err=10,end=11) temp
             !read(iunit,'(a30)',err=10,end=11) temp
	         do i=1,nvar
	            read(iunit,*,err=10,end=11)
	         end do
             ...
             if (transpar_it==num_stp) close(iunit)
             return
			 ...
			 if (transpar_it==1) iunit = generate_unit(100)
             open(iunit,file=file_name)
             read(iunit,*,err=10,end=11)
	         read(iunit,*,err=10,end=11) nvar
	         do i=1,nvar
             ...
             write(*,*) 'Cannot use flag 2 (MF2k) for variables other than flow'
		     stop
          end if
