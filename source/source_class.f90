    ...
    subroutine read_source_ (this,namein,TypeInjIn,fname)
       ...
       character(len=100)                   :: dist(3)
           ...
           case ( 'CELLS_FILE_PARTICLE_NUMBER' )
              ...
              allocate (this % par % file)
              this % par % file = file
			  call read_blocks_part_ (file,ix,iy,iz,np,pmass,dist,npblock)
			       allocate(this % par % ix(size(ix))); this % par % ix = ix
			       allocate(this % par % iy(size(iy))); this % par % iy = iy
			       allocate(this % par % iz(size(iz))); this % par % iz = iz
           ...
           case ( 'MASS_FLUX_PER_CELL' )
              if (this%TypeInj == 'DIRAC') then
		          stop '...reading from cells file (with mass flux per cell) not accessible for DIRAC injections'
              end if
              !ReadFromFile = .TRUE.
			  read(unit,*) pmass,zone,specie
			  read(unit,*) file,const
              ...
              allocate (this % par % file)
              this % par % file = file
              call read_blocks_mf_ (file,const,dist,this%timefunct)
              allocate(this % par % dist_part(3)); this % par % dist_part = dist
				   this % np     = np
				   this % pmass  = pmass
	               this % zone   = zone
           ...
           if (this%typeInj == 'GENERAL') then
              ...
              if (trim(adjustl(name)) /= 'MASS_FLUX_PER_CELL') then
                  read(unit,*) file,const
                  ...
                  this % freq = 1
                  do it=1,this%timefunct%nt
                    if (sum(this%timefunct%val_cell(it)%val_c) /= 0.d0) then
                       this % TimeStartInj = this%timefunct%time(it)
                       exit
                    end if
                  end do
...
!	-------------------------------------------------------------------------------------- 			     
    ...
    subroutine read_blocks_part_ (fname,ix,iy,iz,np,pmass,dist,npblock)
        ...
        allocate(ix(nblock),iy(nblock),iz(nblock),npblock(nblock))
        do i=1,nblock
            read(iunit,*) ix(i),iy(i),iz(i),npblock(i)
        end do
        close(iunit)
    end subroutine
    
    
    subroutine read_blocks_mf_ (fname,scale,dist,func)
        use gslib, only: open_fname_normal,upper_case
        use timefunction_class
        implicit none
        character(len=*),      intent(in)    :: fname
        ...
        func%nt = nt
        func%scale = scale
        allocate(func%time(nt))
        allocate(func%val_cell(nt))
        do i=1,nt
            read(iunit,*) func%time(i)
        end do
        ...
        do i=1,nt
            read(iunit,*) ncell
            allocate(func%val_cell(i)%ncell)
            func%val_cell(i)%ncell = ncell
            allocate(ix(ncell),func%val_cell(i)%ix(ncell))
            allocate(iy(ncell),func%val_cell(i)%iy(ncell))
            allocate(iz(ncell),func%val_cell(i)%iz(ncell))
            allocate(mf(ncell),func%val_cell(i)%val_c(ncell))
            do j=1,ncell
                read(iunit,*) ix(j), iy(j), iz(j), mf(j)
            end do
            func%val_cell(i)%ix = ix
            func%val_cell(i)%iy = iy
            func%val_cell(i)%iz = iz
            func%val_cell(i)%val_c = mf
            deallocate(ix,iy,iz,mf)
        end do
    ...
    subroutine print_source_ (this,fname)
       ...
       integer                              :: unit
           ...
           elseif ( trim(adjustl(name)) == 'MASS_FLUX_PER_CELL' ) then
                   write(unit,'(a26,x,g16.6)') 'particle mass...........: ',this%pmass
	               write(unit,'(a26,x,a30)')   'source type.............: ',this%name
				   write(unit,'(a26,x,a15)')   'injection type..........: ',this%TypeInj
