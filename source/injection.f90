 ...
 subroutine inject_particles ( plume, geo, advection, mass_trans, reaction, source, dt )
       ...
       real*8,                intent(in)    :: dt
       integer                              :: isource,n,nt,i,np, ncell
...
!...loop over injections
       ...
       source_loop: do isource=1,n	  
          ...
          if (trim(adjustl(source%num(isource)%TypeInj)) == 'GENERAL') then              
                 ...
                 if (trim(adjustl(source % num (isource) % name)) == 'MASS_FLUX_PER_CELL') then
                    ncell = source%num(isource)%timefunct%val_cell(source%num(isource)%loc)%ncell
                    allocate(Jm_cell(ncell))
                    Jm_cell = evaluate_timefunction_upwind_cell_ (source%num(isource)%timefunct,source%num(isource)%loc) !get mass flux for each cell (where injection occurs) for the right period                    
                    Mass = sum(Jm_cell)*dt !total mass to inject
                    if (Mass > 0.d0) then
                        !get total number of particles to be injected, and number of particle to inject per cell
                        if (associated(source%num(isource)%par%npblock)) deallocate(source%num(isource)%par%npblock) !clean number of particles to inject per cell
                        allocate(source%num(isource)%par%npblock(ncell))
                        np = max(ncell,int(Mass/source%num(isource)%pmassINI+0.5d0))     !first estimate of the total number of particles to inject, inject at least one particle per cell
                        do i=1,ncell
                            source%num(isource)%par%npblock(i) = max(1,int(np*Jm_cell(i)*dt/Mass+0.5d0))  !number of particles in each cell (at least 1)
                        end do
                        np = sum(source%num(isource)%par%npblock)    !reajust the total number of particles
                        source%num(isource)%np = np
                        source%num(isource)%pmass = Mass/dfloat(source%num(isource)%np)  !correct mass to match mass flux if needed
                        call inject_particles_one_source ( plume, geo, advection, mass_trans, reaction, source, isource )
...
!***********************************************************************************
  subroutine inject_particles_one_source ( plume, geo, advection, mass_trans,reaction, source, kinj )
	   ...
	   real*8, allocatable                :: qm(:), qmm(:,:)
	   logical                            :: Intersection
	   integer                            :: nx,ny,nz,kk,jj,ii
	   integer                            :: npi,npf,jpart
	   integer                            :: izone,ispecie
	   real*8                             :: rpt,Vol
	   integer                            :: id, it
	   real*8, pointer                    :: conc(:) => null()
       integer, allocatable               :: ix(:), iy(:), iz(:), dxv(:), dyv(:), dzv(:), npblockm(:,:), npblockv(:), zinjv(:,:)
...
!       ========================================
        ...
        !***********************************************************    
        ! the injection line is vertical and parallel to x-direction
        !***********************************************************            
        elseif (yinj_1 == yinj_2 .and.  zinj_1 == zinj_2) then
             mp     = source % num (kinj) % pmass         
             np     = source % num (kinj) % np
		     zone   = source % num (kinj) % zone
		     specie = source % num (kinj) % specie 
            ...
            ! find the end point cells defining the line
             ...
             do i=i2,i3
                  ...
                  qy = 0.5d0*(advection%qy%values(i,j2,k2)+advection%qy%values(i,j2+1,k2))
                  if (nz>1) then
                    qz = 0.5d0*(advection%qz%values(i,j2,k2)+advection%qz%values(i,j2,k2+1))
                  else
                    qz = advection%qz%values(1,1,1)
                  end if
                  if (nx /= 1) dx = value_array_ (geo%dx,i,1,1)
                  qm(i) = dsqrt(qy*qy+qz*qz)*dx
             ...
             do i=i2,i3
                 ...
                 if (sum(qm(i:i3)) > 0.d0) npblock = dint( (np-npsum) * qm(i) / sum(qm(i:i3)) + 0.5d0 )    
                 ...
                 do jp=1,npblock
                     ...
                     xp(1) = xx0 + jp * dx / dfloat(npblock+1)
...
!       ================================================
           ...
           qz = advection%qz%values(i2,j2,k2)
           do i=i2,i3
              ...
              do j=j2,j3
                  do k=k2,nz
                      ...
                      if (qz>0.d0) then
                          ...
                          qmm(i,j) = abs(qz)
                          sumq = sumq + qmm(i,j)
                          zinjv(i,j) = geo%zmesh%values(i,j,k)
                          exit
                      end if
                  end do
              end do
	  ...
	  else if ( trim(adjustl(fsour)) == 'MASS_FLUX_PER_CELL' ) then
        ...
        it = source % num (kinj) % loc
        allocate(ix(size(source % num (kinj) % timefunct % val_cell (it) % ix)))
        allocate(iy(size(source % num (kinj) % timefunct % val_cell (it) % iy)))
        allocate(iz(size(source % num (kinj) % timefunct % val_cell (it) % iz)))
	    ix = source % num (kinj) % timefunct % val_cell (it) % ix
	    iy = source % num (kinj) % timefunct % val_cell (it) % iy
	    iz = source % num (kinj) % timefunct % val_cell (it) % iz
        
        allocate(npblockv(source % num (kinj) % timefunct % val_cell (it) % ncell))
        npblockv  = source % num (kinj) % par % npblock
