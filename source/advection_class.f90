  ...
  type poro_info_cl
      ...
      integer           :: ivar,flag
      logical           :: trans
      integer           :: iunit    !save opened file unit if transient
  end type
  ...
  type advection_cl
      ...
      type(poro_info_cl):: poro_info
      !integer           :: iunit
      integer           :: iunit_trans(3)
   end type
...
!			   end if
   ...
   subroutine calculate_advection_timeshots_ (this)
    ...
    subroutine update_advection_timeshot_ (this,time)
       ...
       use global_variables, only: mf2k,dataqx,dataqy,dataqz 
       use trans_par_times
       implicit none
       type(advection_cl),   intent(inout) :: this
       real*8,               intent(in)    :: time
       ...
       integer                           :: iloop
         ...
         if (.not.dataqx%trans .AND. .not.dataqy%trans .AND. .not.dataqz%trans) return
         if (dataqx%flag == 2) then !Fluxes from Modflow binary file
  ...
  subroutine read_fluxes_ (advection,geo) 
     ...
     if (dataqx%flag == 2 .AND. .not.advection%switch) return
     if (.not.transpar_switch) return
...
!         Read flow from GSLIB file:
          if (dataqx%flag == 1 .AND. dataqx%trans) then !keep GSLIB file open if transient conditions
              if (associated(advection%qx%values)) call delete_array_ (advection%qx)
		      call read_array_2_ (dataqx%file,dataqx%ivar,dataqx%const,dataqx%flag,geo%nx+1,geo%ny,geo%nz,advection%qx,advection%iunit_trans(1))
              write(*,*) 'Update qx at t=',transpar_times(transpar_it)
          else
              advection%qx = read_array_ (dataqx%file,dataqx%ivar,dataqx%const,dataqx%flag,geo%nx+1,geo%ny,geo%nz)
          end if
          
          if (dataqy%flag == 1 .AND. dataqy%trans) then !keep GSLIB file open if transient conditions
              if (associated(advection%qy%values)) call delete_array_ (advection%qy)
			  call read_array_2_ (dataqy%file,dataqy%ivar,dataqy%const,dataqy%flag,geo%nx,geo%ny+1,geo%nz,advection%qy,advection%iunit_trans(2))
              write(*,*) 'Update qy at t=',transpar_times(transpar_it)
          else 
              advection%qy = read_array_ (dataqy%file,dataqy%ivar,dataqy%const,dataqy%flag,geo%nx,geo%ny+1,geo%nz)
          end if
          
          if (dataqz%flag == 1 .AND. dataqz%trans) then !keep GSLIB file open if transient conditions
              if (associated(advection%qz%values)) call delete_array_ (advection%qz)
			  call read_array_2_ (dataqz%file,dataqz%ivar,dataqz%const,dataqz%flag,geo%nx,geo%ny,geo%nz+1,advection%qz,advection%iunit_trans(3))
              write(*,*) 'Update qz at t=',transpar_times(transpar_it)
          else 
...
!    calculates x,y,z velocity components at each node 
!*****************************************************************************************       
    subroutine poro_nodes_ (this,geo,cell,poronode)
    use array_class
	use geometry_class
	implicit none
	...
	type (advection_cl), intent(in)  :: this
	integer,             intent(in)  :: cell(3)
	real*8,              intent(out) :: poronode(2,2,2)
	real*8                           :: wx,wy,wz,sum
    integer                          :: m,n,i,j,k,icell,jcell,kcell,nx,ny,nz,loci,locj,lock, test
            ...
            do m=0,1
			  do n=0,1
			      ...
			      if (icell < 1 .or. icell > nx   .or. &
				      ...
				      kcell < 1 .or. kcell > nz ) cycle
					  wx = weightx (geo,icell,jcell,kcell)
                      poronode(i,j,k) = poronode(i,j,k) + wx * value_array_ (this%poro,icell,jcell,kcell)
                      sum = sum + wx
			  end do
			end do
			if (sum > 0.d0) poronode(i,j,k) = poronode(i,j,k) / sum
