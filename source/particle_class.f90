...
!***********************************************************************************************
	  ...
	  integer                          :: nx,ny,nz,i,j,k,iprev,jprev,kprev, test
...
!***************************************************************************************************
    subroutine update_properties_particle_1 (this,geo,advection,dispersion,reaction)
	   ...
	   integer                            :: i, test
       real*8                             :: xx,yy,zz,poronode(2,2,2)
       ...
       !if (poro_homogeneous == .FALSE.) then
       !     xx = this%cell%coord(1)
	   !     yy = this%cell%coord(2)
	   !     zz = this%cell%coord(3)
       !     call poro_nodes_( advection,geo,this%cell%num,poronode )
       !     this%prop%poro = interpolation_trilinear ( poronode,xx,yy,zz )           
       !else
            this%prop%poro = update_property_block_discrete ( this,geo,advection%poro )
       !end if
       ...
       type(well_vect_cl),  optional, intent(in)    :: well
         ...
         !if (vel_homogeneous== .FALSE. .or. nmove == 1) then
		    if (advection%action) then
                if (this%control%switchcell .or. transpar_switch) call vel_faces_ (advection,this%cell%num,this%cell%qxface,this%cell%qyface,this%cell%qzface) 
         ...
         !if (disp_homogeneous== .FALSE. .or. nmove == 1) then
		    if (present(dispersion)) then
		        if (dispersion%action) then
		            if (this%control%switchcell .or. transpar_switch) call vel_nodes_ (advection,geo,this%cell%num,this%cell%qxnode,this%cell%qynode,this%cell%qznode)
...
!**********************************************************************************************
   subroutine update_dispersion_nodes_particle_ (this,geo,dispersion)
	  ...
	  integer                            :: i,j,k
	   ...
	   if (.not. this%control%switchcell .and. .not.transpar_switch) return
...
!****************************************************************************************
    subroutine check_mobility_particle_ (this, advection, dispersion)
	   ...
	   use array_class
	   use gslib, only: open_fname
	   use global_variables, only: fdbg,npStuck, mf2k,dataqx,dataqy,dataqz
	   ...
	   implicit none
		 ...
		 integer                            :: unit  
		   ...
		   if ( this%vel%qpL(1) == 0.d0 .and. &
                ...
                if (dataqx%trans .OR. dataqy%trans .OR. dataqz%trans) return !transient velocity: particle might keep moving later on
